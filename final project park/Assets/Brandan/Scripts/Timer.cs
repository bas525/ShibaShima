...
public class Timer : MonoBehaviour {
	...
	public int timer;
	private int maxTime;
	public float freeOver;
	public AudioSource bgm;
	public float visibleVol;
	[Header("Players")]
	...
	*/
	...
	private float nextGoodMushroom = 5;
	...
	private float nextDogBone = 2;
	public int dogBoneTotal;
	
	[Header("Spawn Chocolate")]
	public GameObject[] Chocolates;
	public float chocolateRespawn;
	private float nextChocolate = 2;
	public int chocolateTotal;

	// Use this for initialization
	void Start () {
		maxTime = (int)Time.time+120;
		freeOver = Time.time+150;
	}
	
	// Update is called once per frame
	void Update () {		
		if(freeOver < Time.time) {
			Application.LoadLevel("Game_Over");
		}
		
		if(freeOver - Time.time < 5) {
			bgm.volume = (freeOver-Time.time)/5f;
			visibleVol = bgm.volume;
		}
